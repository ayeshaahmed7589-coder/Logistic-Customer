...
//     print("Product Type ID: ${cache["selected_product_type_id"]}");
//     print("Packaging Type ID: ${cache["selected_packaging_type_id"]}");
//     print("Service Type ID: ${selectedServiceTypeId}"); // ✅ Yahaan add karo
...
//       final quantity =
//           int.tryParse(cache["stop_${i}_quantity"]?.toString() ?? "1") ?? 1;
//       final weight =
//           double.tryParse(cache["stop_${i}_weight"]?.toString() ?? "50") ??
//           50.0;
...
//   //     final quantity =
//   //         int.tryParse(cache["stop_${i}_quantity"]?.toString() ?? "0") ?? 0;
//   //     final weight =
//   //         double.tryParse(cache["stop_${i}_weight"]?.toString() ?? "0") ?? 0.0;

//   //     // Get coordinates (use defaults if not available)
//   //     final lat =
//   //         double.tryParse(
//   //           cache["stop_${i}_latitude"]?.toString() ?? "-26.2041",
//   //         ) ??
//   //         -26.2041;
//   //     final lng =
//   //         double.tryParse(
//   //           cache["stop_${i}_longitude"]?.toString() ?? "28.0473",
//   //         ) ??
//   //         28.0473;
...
//   //   // Get service type and declared value
//   //   final serviceType = selectedServiceTypeId ?? "standard";
//   //   final declaredValueStr = cache["declared_value"]?.toString() ?? "0";
//   //   final declaredValue = double.tryParse(declaredValueStr) ?? 0.0;

//   //   // Get total weight from cache as fallback
//   //   final totalWeightKg =
//   //       double.tryParse(cache["total_weight"]?.toString() ?? "0") ??
//   //       totalWeight;
...
//   //             ? totalQuantity
//   //             : 1, // ✅ ADD DEFAULT QUANTITY
//   //         weight: totalWeight > 0
//   //             ? totalWeight
//   //             : totalWeightKg, // ✅ USE TOTAL WEIGHT
//   //         serviceType: serviceType,
//   //         declaredValue: declaredValue,
//   //         addOns: selectedAddons,
//   //       );
...
//       print("🎯 Placing Order...");
//       print("Order Type: ${isMultiStop ? 'Multi-Stop' : 'Standard'}");

//       final repository = ref.read(placeOrderRepositoryProvider);
//       OrderResponse orderResponse;

//       // Call the appropriate order placement method directly
//       if (isMultiStop) {
...
//         // Navigate to order success screen and remove all previous screens
...
//             ),
//           ),
//           (route) => false, // Sab previous routes clear ho jayenge
//         );
//       } else {
...
// //                           child: GridView.builder(
// //                             itemCount:
// //                                 addOnItems.length, // ✅ data.addOns → addOnItems
// //                             gridDelegate:
// //                                 const SliverGridDelegateWithFixedCrossAxisCount(
// //                                   crossAxisCount: 2,
// //                                   crossAxisSpacing: 12,
...
// //                           return serviceTypeState.when(
// //                             data: (serviceItems) {
// //                               // ✅ Directly get List<ServiceTypeItem>
// //                               return Column(
// //                                 children: serviceItems.map((service) {
// //                                   final basePrice = 100.0;
// //                                   final calculatedPrice =
// //                                       basePrice *
// //                                       service
// //                                           .priceMultiplier; // ✅ Changed to priceMultiplier
// //                                   final priceText =
// //                                       service.priceMultiplier > 1.0
// //                                       ? "(+R${(calculatedPrice - basePrice).toStringAsFixed(0)})"
// //                                       : "(R${calculatedPrice.toStringAsFixed(0)})";
// //                                   return _serviceOption(
// //                                     selected:
// //                                         selectedServiceTypeId ==
// //                                         service.value, // ✅ Changed to value
// //                                     title: service.label, // ✅ Changed to label
// //                                     subtitle:
// //                                         "${service.description} $priceText",
// //                                     value: service.value, // ✅ Changed to value
// //                                     icon: service.getIconData(),
// //                                     multiplier: service
// //                                         .priceMultiplier, // ✅ Changed to priceMultiplier
// //                                   );
...
// //                       final addOnsState = ref.watch(addOnsControllerProvider);
// //                       return addOnsState.when(
// //                         data: (addOnItems) => _buildAddOnsSection(
// //                           addOnItems,
// //                         ), // ✅ Direct list pass
// //                         loading: () => _buildAddOnsLoading(),
// //                         error: (error, stackTrace) => _buildAddOnsError(),
...
// //         // ✅ UPDATED FIELDS
// //         final isSelected = selectedAddons.contains(item.value); // id → value
// //         final dynamicCost = item.calculateCost(declaredValue);
// //         final isPercentage = item.priceType == 'percentage'; // type → priceType
// //         final basePrice = item.price; // cost → price
...
// //             borderRadius: BorderRadius.circular(16),
// //             onTap: () {
// //               _toggleAddOn(item.value, dynamicCost); // id → value
// //             },
// //             splashColor: AppColors.electricTeal.withOpacity(0.2),
// //             highlightColor: AppColors.electricTeal.withOpacity(0.1),
// //             child: AnimatedContainer(
...
// //                             padding: const EdgeInsets.only(left: 2),
// //                             child: Text(
// //                               "(${item.price.toStringAsFixed(0)}%)", // ✅ rate → price
// //                               style: TextStyle(
// //                                 fontSize: 9,
// //                                 color: isSelected
// //                                     ? Colors.white.withOpacity(0.9)
...
Future<void> _calculateMultiStopQuotes({
  ...
  ref.read(orderCacheProvider.notifier).saveValue("quantity", totalQuantity.toString());
  ref.read(orderCacheProvider.notifier).saveValue("total_weight", totalWeight.toString());
  
  print("💾 Saved to cache: quantity=$totalQuantity, total_weight=$totalWeight");
  ...
  try {
    ...
    if (isMultiStop) {
      ...
      if (quantity == null || quantity.isEmpty || int.tryParse(quantity) == 0) {
        ...
        for (int i = 1; i <= stopsCount; i++) {
          ...
          final wgt = double.tryParse(cache["stop_${i}_weight"]?.toString() ?? "50") ?? 50.0;
        ...
        // Save to cache
        ref.read(orderCacheProvider.notifier).saveValue("quantity", totalQty.toString());
        ref.read(orderCacheProvider.notifier).saveValue("total_weight", totalWeight.toString());
    ...
    if (isMultiStop) {
      ...
      if (request.quantity < 1 || request.weightPerItem < 0.01) {
        ...
        final fixedRequest = MultiStopOrderRequestBody(
          ...
          weightPerItem: request.weightPerItem < 0.01 ? 0.01 : request.weightPerItem,
          isMultiStop: true,
        ...
        orderResponse = await repository.placeMultiStopOrder(request: fixedRequest);
      } else {
        orderResponse = await repository.placeMultiStopOrder(request: request);
      }
    } else {
    ...
    if (errorMsg.contains("quantity") || errorMsg.contains("weight_per_item")) {
      errorMsg = "Please ensure all stops have at least 1 item and valid weight. "
...
void _debugCacheData() {
  ...
  // Standard order keys
  print("\nSTANDARD ORDER KEYS:");
  print("quantity: ${cache["quantity"]} (Type: ${cache["quantity"]?.runtimeType})");
  print("total_weight: ${cache["total_weight"]} (Type: ${cache["total_weight"]?.runtimeType})");
